...
def parse_args():
    ...
    # Required parameters
    parser.add_argument("--fragment_file", type=str, default=None, help="input fragment file")
    parser.add_argument("--peak_file", type=str, default=None, help="input peak file")
...
def main():
    ...
    logging.info(f"Total of {len(chrom_sizes)} chromosomes")

    open_fn = gzip.open if args.fragment_file.endswith(".gz") else open
    skip_rows = 0
    with open_fn(args.fragment_file, "rt") as f:
        for line in f:
            line = line.strip()
    ...
    logging.info(f"Reading fragments from {args.fragment_file}")
