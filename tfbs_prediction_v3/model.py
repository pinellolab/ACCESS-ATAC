...
class ACCESSNet(nn.Module):
    def __init__(
        ...
        self.atac_signal_embd = nn.Sequential(nn.Linear(self.peak_len, 32), 
                                              nn.ReLU(), 
                                              nn.Dropout(self.dropout_rate), 
                                              nn.Linear(32, self.embd_len))
        
        self.access_signal_embd = nn.Sequential(nn.Linear(self.peak_len, 32), 
                                                nn.ReLU(), 
                                                nn.Dropout(self.dropout_rate), 
                                                nn.Linear(32, self.embd_len))
    ...
    def forward(self, peak_seq, atac_signal=None, access_signal=None):
        # Embed peak sequence
        ...
        if atac_signal is not None:
            ...
            seq_embd = seq_embd + atac_signal 

        if access_signal is not None:
            access_signal = self.access_signal_embd(access_signal)
            seq_embd = seq_embd + access_signal 
